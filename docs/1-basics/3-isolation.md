...
例えばこれらの機能を使い「`/tmp`がこれからは`/`だ」と設定すれば、設定されたプロセスは`/tmp`以下のファイルしか閲覧できなくなります。

これも**プロセス単位**の概念で、あるプロセスのルートを変更すれば、その**子プロセスも**自動的に**そこがルートになり**ます。
...
今回のワークショップでは扱わないため、是時各自で調べてみて下さい。
